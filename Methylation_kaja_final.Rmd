...
#go1 = methylRRA(cpg.pval = cpg.pval, method = "GSEA", 
 #                   minsize = 50, maxsize = 5000, GS.type= NULL, GS.list = pathgsea  , array.type = "EPIC" )
...
# add ERV here 
# first get from ucsc repeat masker and coordinates. 
library (  GenomicRanges )
...
#### DM trends for ERV
  * Are do probes on ERV elements show similar trend? 
   + Method: ERV coordinates donwloaded from UCSC repeatMasker. Only Family of ERV elements were included, LINC, SINE were excluded. 
   + Each position of Cpg probes ( from EPIC plateform) were added and subtracted by 1 bp  
   + These probes were chosen if the geneomic position fall within the range of ERV.  
...
```{r fig=TRUE,fig.width=8, fig.height=6.5, echo=FALSE, include=TRUE, results='asis' }
gseaGo_devel_plot
```
...
gseaGo_all_plot= ggplot(cluegoALLb , aes( x = group, log2(total) )) + 
...
#### plot PCG
  ...
  * we do a total GSEA pathways, this includes kegg, go, and reactome 
  * only groups with >5 pathways are shown
  * each group is described by the pathway with the lowest fdr follow by highest enrichment score.
  * total is represented as log2 ( total pathways for the particular group)

```{r fig=TRUE,fig.width=12, fig.height=6.5, echo=FALSE, include=TRUE, results='asis' }
gseaGo_all_plot
...
### adding LUAD TSG here not in kaja list. 
# S_Table 5-Verification
...
msgtsgkras = paste ( 
  ...
  "correlated with KRAS, or of these", total_uhrf1KRAS_cor_neg, "were significantly anti correlated, or ", round ( total_uhrf1KRAS_cor_neg/total_uhrf1KRAS_cor,2), "percent"
  )
...
### HR table + TSG {.tabset}
  ...
  * the following censors are analyzed: "OS","DSS","DFI","PFI"
    + 4 different survival were completed here.
      - OS, DSS ( Overall Specific Survival, death specific to disease aka not run over by car)
      - PFI as well as DFI, progression and disease free
  * __ALSO__ in TSG list that correlated negatively with UHRF1
...
## add zheng - combine all negative and see what the results are. 
...
#### combined TSG and survival {.tabset}
  ...
  * Now we take all the "protective" genes in either OS or KRAS, combined them and look for survival 
...
#### Table
  ...
  * note __CDKN1A does have OR < 1 __ 
...
klist = c ( "FHL1", "DLC1", "DUSP22", "LIMD1", "AXIN2", "PTEN", "HBP1", "NR4A1", "NR4A3", "CUX1", "CACNA2D3", "BNIP3L", "KRAS" ) 
