...
# remove any genes that also has wt p < .05
wt_hr_genes = surv_kras_tsg_cor_uhrf1[surv_kras_tsg_cor_uhrf1$type %in% c ( "wt"), ]$gene 
...
surv_kras_tsg_cor_uhrf1 [ surv_kras_tsg_cor_uhrf1$censor == "PFI", ] %>%
  ggplot(aes(x = HR,
             y = gene)) +
...
freq = data.frame ( table ( onlykras [ onlykras$gene %in%  row.names ( tally_cor ) ,  c("censor", "type")]))
freq = freq [ order ( freq$Freq, decreasing = T ), ]
...
# analyzed survival for only kras only survival + TSG 

surv_kras_tsg_cor_uhrf1 = onlykras [ onlykras$gene %in%  row.names ( tally_cor ) ,  ]
